...

           {
             ...
             render: function(data, type, row) {
               <?php if ($this->authlibrary->HasModulePermission('SAMPATI-RASID', 'VIEW')) { ?>
                 ...
                 if (row.status == 'सदर') {
                   var res = '<a class="btn btn-warning btn-sm" href = "<?php echo base_url() ?>SampatiKarRasid/newPreviewBills/' + row.bill_no_en + '/' + row.nb_file_no_en + '/' + row.nb_fiscal_year + '" target="_blank">विवरण हेर्नुहोस</a>';
                   return res;
                 } else {
